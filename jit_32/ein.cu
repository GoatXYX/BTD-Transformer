...
__global__ void ein_backward(
    ...
    if (fabsf(go) < 1e-8) {  // 判断梯度输出是否异常小
        go += 1e-5;
        // printf("Very small grad_output detected at block(%d, %d, %d) thread(%d, %d, %d) with value: %f\n",
        //        bx, by, bz, tx, ty, tz, go);
        // printf("core_value: %f, rw_head_q: %f, w_head_k: %f, w_head_v: %f\n",
        //        core_value[0], rw_head_q[0], w_head_k[0], w_head_v[0]);
    }
    int base_q = i * DIM_B * DIM_H;
    int base_k = j * DIM_B * DIM_H;
    int base_v = k * DIM_B * DIM_H;
...
void launch_ein_backward(
    ...
    cudaError_t errorBackward = cudaGetLastError();
    if (errorBackward != cudaSuccess) {
        printf("CUDA error in backward: %s\n", cudaGetErrorString(errorBackward));
    }
}
