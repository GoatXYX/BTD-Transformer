...
parser.add_argument('--tgt_len', type=int, default=32,
                    help='number of tokens to predict')
parser.add_argument('--eval_tgt_len', type=int, default=32,
...
###############################################################################
'''
使用了 get_lm_corpus 函数加载数据。根据数据集类型（如 ptb、wt103），返回一个 Corpus 对象，该对象提供以下功能：
加载 train.txt, valid.txt, 和 test.txt 数据。
根据 batch_size 和 tgt_len 生成迭代器。
'''
corpus = get_lm_corpus(args.data, args.dataset)
ntokens = len(corpus.vocab)  # size of vocabulary
args.n_token = ntokens  # size of vocabulary
...
###############################################################################
'''
定义了一个 TensorizedTransformerLM 模型，并初始化参数。

主要组件：

Transformer 模型的核心结构，包括多头注意力、前馈层等。
自适应 softmax，用于高效处理大词汇表（args.adaptive=True 时启用）。
'''
...
def evaluate(eval_iter):
    '''
    evaluate 函数的核心任务 是评估模型在验证集或测试集上的性能。
    它采用禁用梯度的模式，节省计算资源，同时通过动态调整模型的上下文长度，确保评估与训练时的配置一致。
    函数灵活性高，可扩展用于其他任务（如分类或翻译）。
    '''
