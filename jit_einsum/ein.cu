...
__global__ void ein(float* core_value, float* rw_head_q, float* w_head_k, float* w_head_v, float* output,
...
void launch_ein(float* core_value, float* rw_head_q, float* w_head_k, float* w_head_v, float* output, int DIM_I, int DIM_J, int DIM_K, int DIM_B, int DIM_H) {
    dim3 grid((DIM_I + TM - 1) / TM, (DIM_J + TN - 1) / TN, (DIM_K + TN - 1) / TN * DIM_B);
    dim3 block(4, 4, 4);
    ein<<<grid, block>>>(core_value, rw_head_q, w_head_k, w_head_v, output, DIM_I, DIM_J, DIM_K, DIM_B, DIM_H);
}
